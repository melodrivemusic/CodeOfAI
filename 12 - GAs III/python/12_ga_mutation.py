...
def mutate(chromosome, mutationChance=0.1):
    """
...
def mutatePop(population, mutationChance=0.1):
    """
    Mutate a population by flipping bits on the chromosomes
    :param population:
    :param mutationChance:
    :return:
    """
    ...
    for chromosome in population:
        mutate(chromosome, mutationChance)


def gaKnapsack(items, capacity, popSize=20, numGenerations=20, mutationChance=0.01):
    ...
    for generation in range(numGenerations):
        ...
        # 4) Mutation with elitism
        # keep the best performing chromosome
        best = population[0]
        # mutate the rest
        rest = population[1:]
        mutatePop(rest, mutationChance)
        population = [best] + rest
...
if __name__ == "__main__":
    ...
    for p in popSizes:
        results[p] = {}
        for m in mutationChances:
            result = gaKnapsack(items, capacity, popSize=p, mutationChance=m)
            results[p][m] = result

    print("{:10} | {:10} | {:10}".format("popSize", "mutation", "bestValue"))
    print("=" * 36)
    for p in popSizes:
        for m in mutationChances:
            bestCombination, bestValue, bestWeight = results[p][m]
            print("{:10} | {:10} | {:10}".format(p, m, bestValue))
        print("=" * 36)
