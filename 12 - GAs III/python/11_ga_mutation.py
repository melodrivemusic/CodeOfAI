...
def mutate(population, mutationChance=0.1):
    if mutationChance == 0:
        return
...
def gaKnapsack(items, capacity, popSize=50, numGenerations=20, mutationChance=0.01):
    ...
    for generation in range(numGenerations):
        ...
        # mutate the rest
        rest = population[1:]
        mutate(rest, mutationChance)
        population = [best] + rest

        # 5) Calculate fitness for our new population
        fitnessScores = getFitness(population, items, capacity)
